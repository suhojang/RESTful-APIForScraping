...
public class StringUtil {
    ...
    public static String remove(String src, String oldstr) {
    	return replace(src,oldstr,"");
    }	
    public static String removeLine(String src){
    	String dest	= replace(src,"\r","");
    	dest	= replace(dest,"\n","");
    	return dest;
    }
	...
	*/
	...
	public static final String convertBzNo(String bzno){
		if(bzno==null)
			return "";
		bzno	= replace(bzno,"-","");
		
		if(bzno.length()==10){
			return bzno.substring(0,3)+"-"+bzno.substring(3,5)+"-"+bzno.substring(5);
		}else{
			return bzno;
		}		
	}
	public static final String convertSSn(String ssn){
		if(ssn==null)
			return "";
		ssn	= replace(ssn,"-","");
	...
	//문자열을 특정 바이트 크기만큼으로 잘라서 list에 담아 반환
	public static final List<String> cutBytes(String val,int limitSize,String encoding) throws Exception{
		List<String> list	= new ArrayList<String>();
		if(val==null || "".equals(val))
			return list;
		
		if(val.getBytes(encoding).length<=limitSize){
			list.add(val);
			return list;
		}
		int sz	= 0;
		...
		String c	= null;
		for(int i=0;i<val.length();i++){
			c	= String.valueOf(val.charAt(i));
			sz	+= c.getBytes(encoding).length;
			if(sz>limitSize){
				list.add(sb.toString());
				sz	= c.getBytes(encoding).length;
				sb.setLength(0);
    ...
    public static final boolean isContain(Object[] arr, Object obj){
     ...
     * 숫자여부 판단 (- 가능)
     * */
    public static boolean isValidInt(Integer val) {
    	return true;
    }
    ...
    public static boolean isValidTime(String str) {
		String time	= StringUtil.replace(str,":","");
        for (int i = 0; i < time.length(); i++) {
            if (Character.isDigit(time.charAt(i)) == false)
                return false;
        }
		if(time.length()!=4)
			return false;
    ...
    public static String cardNoFormat(String num){
    	if(num==null)
    		return "";
    	...
    	num	= addChar(num,16,"*",false);
   		num	= num.substring(0, 4)+"-"+num.substring(4, 8)+"-"+num.substring(8, 12)+"-"+num.substring(12);
    	return num;
    }
	...
	public static boolean isNumeric(String val) {
		return val.matches("^[\\-]{0,1}[0-9]*$");
	}
	public static boolean isPositiveNumeric(String val) {
		return val.matches("^[0-9]*$");
	}
	public static boolean isNegativeNumeric(String val) {
		return val.matches("^[\\-]{1}[0-9]*$");
	}
	public static boolean isDecimal(String val) {
		return val.matches("^[\\-]{0,1}[0-9]*[\\.]{0,1}[0-9]*$");
	}
	public static boolean isPositiveDecimal(String val) {
		return val.matches("^[0-9]*[\\.]{0,1}[0-9]*$");
	}
	public static boolean isNegativeDecimal(String val) {
		return val.matches("^[\\-]{1}[0-9]*[\\.]{0,1}[0-9]*$");
	}
	
	public static String getMasterKey(int blockSize) {
		StringBuffer sb		= new StringBuffer();
