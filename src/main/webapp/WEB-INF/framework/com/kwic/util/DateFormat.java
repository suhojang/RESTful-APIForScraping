...
public class DateFormat {
	 ...
	 * 기호를 제외한 순수 숫자 반환
	 * */
	public static final String getPureDate(String date){
    ...
    public static boolean isValidDate(String date,String format){
    	...
    	format	= StringUtil.remove(StringUtil.remove(StringUtil.remove(StringUtil.remove(StringUtil.remove(format, "-"),"."),"/")," "),":");
    	SimpleDateFormat sf = new SimpleDateFormat(format);
    	sf.setLenient(false);
    	try{
    		sf.parse(date);
    		return sf.parse(date,new ParsePosition(0)) == null ? false : true;
    	}catch(Exception Ex){
    		return false;
    	}
    }
	 ...
	 * 일자 유효성 검증
	 * */
    public static boolean isValidMonth(String yyyyMM,String format){
    	yyyyMM	= getPureDate(yyyyMM);
    	if(yyyyMM.length()!=6)
    		return false;
    	SimpleDateFormat sf = new SimpleDateFormat(format);
    	sf.setLenient(false);
    	try{
    		sf.parse(yyyyMM);
    		return sf.parse(yyyyMM,new ParsePosition(0)) == null ? false : true;
    	}catch(Exception Ex){
    		return false;
    	}
    }
	...
	public static void main(String[] args){
		String date	= "00010201";
		System.out.println(isValidDate(date,"yyyyMMdd"));
		
		
//		System.out.println(new DateFormat("yyyyMMddHHmm").format("201905222322", "yyyy-MM-dd HH:mm"));
//		System.out.println(DateFormat.isValidDate("201905222322", "yyyy-MM-ddHHmm"));
	}
}
