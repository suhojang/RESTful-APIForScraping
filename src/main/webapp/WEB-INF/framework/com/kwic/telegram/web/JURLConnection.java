...
/**<pre>
* Cilent 에서 서버로의 URLConnection을 담당</pre>
*
...
//	public static String connect( String webUrl,String sendData) throws Exception {
//		
//		URL				url			= null;
...
//	}
	...
	/**<pre>
	* URL Connection 으로 데이터 송신 및 수신을 실시한다.</pre>
	*@param conn DB Connection <--  getConnection()
	*@param flag UpLoad or DownLoad (see - JURLConnection._UP,JURLConnection._DOWN )
	*@param data 전송할 stream 문자열
	*@return String
	*@throws Exception
	*/
	public static String connect( String webUrl,String sendData, Map<String,String> paramMap,int timeout) throws Exception {
		...
		try{
			br						= new BufferedReader( new InputStreamReader( 
											(sendData==null||"".equals(sendData.trim()))
												?sendPost(webUrl,  paramMap,timeout)
												:sendGet(webUrl, sendData, paramMap)
										) );
...
//System.out.println(line);
			...
			try{if(br!=null){br.close();br=null;}}catch(Exception e){br=null;}
		}
		return sb.toString().trim();
	}
	/**<pre>
	...
	*@throws Exception
	*/
	public static byte[] connect( String webUrl,String sendData, Map<String,String> paramMap,int timeout, boolean b) throws Exception {
		
		InputStream is				= null;
		JOutputStream jos			= null;
		byte[]			rtnBytes	= null;
		try{
			is	= (sendData==null||"".equals(sendData.trim()))?sendPost(webUrl,  paramMap,timeout):sendGet(webUrl, sendData, paramMap);
	...
	*/
	private static InputStream sendGet(String urlStr, String sendStream, Map<String,String> paramMap) throws Exception {
		...
		try{
			...
			try{if(pw!=null)pw.close();}catch(Exception e){pw=null;}
		}
		return is;
	}
	/**
	...
	* @return InputStream
	*/
	public static InputStream sendPost(String urlStr, Map<String,String> paramMap,int timeout) throws IOException {
		String paramString	= encodeString(paramMap);
		if (paramString==null || "".equals(paramString.trim()))
			paramString = "";
		...
		try {
			...
			for (int i=0;i<nameArr.length;i++) {
				...
				try{
					...
					try{if(bi!=null)bi.close();}catch(Exception e){bi=null;}
				}
				out.write(newLineBytes);
