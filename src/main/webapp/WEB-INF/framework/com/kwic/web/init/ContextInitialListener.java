...
import com.kwic.web.schedule.ScheduleDemon;
...
public class ContextInitialListener extends ContextLoaderListener{
	...
	public void contextInitialized(ServletContextEvent event) {
		 ...
		 */
		try {
			logger.debug("===== Task schedule create startup!! =====");
			//ScheduleDemon.getInstance().startup();
		} catch (Exception e1) {
			logger.error("Task Schedule create fail.", e1);
		}
	...
	public void contextDestroyed(ServletContextEvent event) {
		...
		//스케쥴 데몬 종료
		try {
			//ScheduleDemon.getInstance().shutdown();
		} catch (Exception e) {
			logger.error("Task Schedule shutdown failed.", e);
		}
