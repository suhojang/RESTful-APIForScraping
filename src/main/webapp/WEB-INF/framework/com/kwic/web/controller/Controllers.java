...
import com.kwic.web.schedule.task.Task_manageips;
...
public class Controllers {
	...
	public static final String RT_CD	= "resultCode";
	public static final String RT_MSG	= "resultMsg";
	public static final String AGT_STATUS	= "status";
	public static final String AGT_MESSAGE	= "message";
	public static final String AGT_ERROR	= "error";
...
//			obj.put(RESULT_MSG, "정상처리되었습니다.");
	 ...
	 */
	public void validateToken(HttpServletRequest request) throws DefinedException {
		DefinedException e	= null;
		String token		= request.getHeader("Authorization");
		if (token==null) {
			e	= new DefinedException("Access Token이 누락 되었습니다.");
			e.setStatusCode(ErrorCode._901);
		} else {
			token	= token.replace("Token ", "");
			if (!token.equals(properties.getString("agent.token"))) {
				e	= new DefinedException("Access Token이 유효하지 않습니다.");
				e.setStatusCode(ErrorCode._902);
			}
		}
	 ...
	 * @throws DefinedException
	 */
	public void validateRemoteIP(HttpServletRequest request) throws DefinedException {
		DefinedException e	= null;
		if (Task_manageips.getManageIps().indexOf("[ALL]") < 0 && Task_manageips.getManageIps().indexOf("["+getRemoteIP(request)+"]")<0) {
			e	= new DefinedException("요청하신 IP["+getRemoteIP(request)+"]는 서버에 접근 가능한 IP주소가 아닙니다.");
			e.setStatusCode(ErrorCode._903);
		}
	 ...
	 * @throws DefinedException
	 */
	public void validate(HttpServletRequest request) throws DefinedException {
		boolean yn1	= true;
		boolean yn2	= true;
		DefinedException e	= null;
		try {
		...
		if(!yn1 && !yn2) {
			e	= new DefinedException("접근 허용 제한[접근 불가능 IP 혹은 Token정보 누락/유효하지 않은 Token]");
			e.setStatusCode(ErrorCode._904);
			throw e;
		}
	}
	...
	protected Map<String,Object> getLogData(Map<String,Object> responseMap, String requestContent, HttpServletRequest request) throws DefinedException, JsonParseException, JsonMappingException, IOException {
		...
		logParam.put("log_req", 	requestContent);
		logParam.put("log_res", 	new ObjectMapper().writeValueAsString(responseMap));
